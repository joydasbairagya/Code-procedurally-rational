...
start=time.time()
#We haveassume first player average belif is x and 2nd player's is y
@njit(nogil=True)
def PR1(x,y,s,t):
	#Procedural player's best response if she is in the position
	# of first player
	if s>t:
		sam_func=y*(2-y)-x
	if s<t:
...
@njit(nogil=True)
def VR2(x,y,s,t):
	#VNM player's best response if she is in the position
	# of second player
	if x+s*(1-x) > t*x:
		best_func= 1-y
	if x+s*(1-x) < t*x:
...
def initial_condition(VR1,VR2,s,t):
	# dependence of initial condition
	X0=np.linspace(0,1,480)
	Y0=np.linspace(0,1,480)
	...
	for i in prange(len(X0)):
		for j in prange(len(Y0)):
			...
			Intitial_condition_dependence[i,j]=time_evolution(VR1,VR2,X0[i],Y0[j],s,t)[0]
	return(np.mean(Intitial_condition_dependence))

# nash=(0.25)/(1+0.25-0.5)
# plt.imshow(initial_condition(VR1,VR2,-0.25,0.5),origin='lower',extent=[0,1,0,1])
...
np.savetxt('Result_parameter_dependence.txt',result)
